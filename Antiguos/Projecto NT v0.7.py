...
def reemplazar_teclas(caracter_final, rango_borrado):
    guardar_datos_deshacer(buffer_lista, caracter_final, rango_borrado)
...
spinbox_margen = tk.Spinbox(frame_general, from_=2, to=4, width=3)       #Configuración del Buffer 
...
capturar_deshacer = tk.Button(frame_deshacer_controles, text="Capturar", command=activar_captura_deshacer, width=8)
capturar_deshacer.grid(row=0, column=1, padx=2)

limpiar_deshacer = tk.Button(frame_deshacer_controles, text="Limpiar", command=limpiar_deshacer_tecla, width=8)
...
tk.Label(ventana_acerca_de, text="Versión 0.7").pack()
