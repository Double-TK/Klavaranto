...
if os.path.exists(config_file):
    ...
    if psutil.pid_exists(pid):
        try:
            ...
            if proceso.name() in ["python.exe", "ProyectoNT.exe", "Projecto NT.exe"]:
                sys.exit()
        except:
            pass
...
imagen_icono_activado = crear_icono('#009000')      # Ícono verde — programa activo
imagen_icono_desactivado = crear_icono('#900000')   # Ícono rojo — programa inactivo
...
def validar_color_afijo(entry):
    letra = entry.get().lower()
    if letra in ["c", "g", "h", "j", "s", "u"]:
        entry.config(bg="red", fg="white")
    else:
        entry.config(bg="#f0f0f0", fg="black")
...
def registrar_tecla_atajo(tecla):                  # Registra las teclas del atajo mientras se captura
    global teclas_atajo
...
def ejecutar_deshacer_v2(backspaces):              # Borra el carácter especial y escribe las letras originales
    global buffer_deshacer_v2, buffer_congelado
...
def iniciar_interfaz():
    ...
    vcmd = ventana_opciones.register(lambda P: len(P) <= 1)
    entry_prefijo_estilo = tk.Entry(frame_estilos, width=3, validate="key", validatecommand=(vcmd, "%P"), justify="center", insertwidth=0, insertontime=0)
    entry_prefijo_estilo.bind("<Key>", lambda e: e.widget.select_range(0, tk.END))
    entry_prefijo_estilo.bind("<KeyRelease>", lambda e: validar_color_afijo(entry_prefijo_estilo))
    ...
    entry_sufijo_estilo = tk.Entry(frame_estilos, width=3, validate="key", validatecommand=(vcmd, "%P"), justify="center", insertwidth=0, insertontime=0)
    entry_sufijo_estilo.bind("<Key>", lambda e: e.widget.select_range(0, tk.END))
    entry_sufijo_estilo.bind("<KeyRelease>", lambda e: validar_color_afijo(entry_sufijo_estilo))
    ...
    ##--------------------------------------------------
    ## SECCIÓN DESHACER - checkboxes y tecla personalizada
    ##--------------------------------------------------
    ...
    checkbox_deshacer_afijo = tk.Checkbutton(frame_estilos, text=textos_idioma[idioma]["deshacer_afijo"], variable=var_deshacer)
    checkbox_deshacer_afijo.grid(row=4, column=0)
    Tooltip(checkbox_deshacer_afijo, textos_idioma[idioma]["tooltip_deshacer_afijo"])

    checkbox_deshacer_tecla = tk.Checkbutton(frame_estilos, text=textos_idioma[idioma]["deshacer_tecla"], variable=atajo_deshacer)
    checkbox_deshacer_tecla.grid(row=5, column=0, sticky="w")
    Tooltip(checkbox_deshacer_tecla, textos_idioma[idioma]["tooltip_deshacer_tecla"])
    ...
    etiqueta_deshacer = tk.Label(frame_atajo_deshacer, text=config['general']['atajo_deshacer'], width=8, justify="center")
    ...
    etiqueta_atajo = tk.Label(frame_atajo_captura, text=config['general']['atajo'], width=20, justify="center")
    ...
    tk.Label(ventana_acerca_de, text=NOMBRE, font=("Arial", 14, "bold")).pack(pady=5)
    tk.Label(ventana_acerca_de, text="Versión " + VERSION).pack()
    tk.Label(ventana_acerca_de, text="Autor: " + AUTOR).pack()
    tk.Label(ventana_acerca_de, text=DESCRIPCION).pack(pady=5)
    ...
    ##--------------------------------------------------
    ventana_opciones.mainloop()        # Mantiene la ventana abierta
