...
def guardar_opciones():
    ...
    config['deshacer']['atajo_deshacer'] = teclas_atajo_deshacer[0].name if teclas_atajo_deshacer and hasattr(teclas_atajo_deshacer[0], 'name') and teclas_atajo_deshacer[0].name else config['deshacer']['atajo_deshacer']
...
#[ceicnt] Construye el nombre de las teclas para los atajos
def construir_nombres_teclas():
    global nombres_teclas
    nombres_teclas = {
        "ctrl_l": textos_idioma[idioma]["tecla_ctrl_l"],
        "ctrl_r": textos_idioma[idioma]["tecla_ctrl_r"],
        "alt_l": textos_idioma[idioma]["tecla_alt_l"],
        "alt_r": textos_idioma[idioma]["tecla_alt_r"],
        "shift": textos_idioma[idioma]["tecla_shift"],
        "shift_l": textos_idioma[idioma]["tecla_shift_l"],
        "shift_r": textos_idioma[idioma]["tecla_shift_r"],
        "caps_lock": textos_idioma[idioma]["tecla_caps_lock"],
        "tab": textos_idioma[idioma]["tecla_tab"],
        "enter": textos_idioma[idioma]["tecla_enter"],
        "backspace": textos_idioma[idioma]["tecla_backspace"],
        "delete": textos_idioma[idioma]["tecla_delete"],
        "esc": textos_idioma[idioma]["tecla_esc"],
        "space": textos_idioma[idioma]["tecla_space"],
        "up": textos_idioma[idioma]["tecla_up"],
        "down": textos_idioma[idioma]["tecla_down"],
        "left": textos_idioma[idioma]["tecla_left"],
        "right": textos_idioma[idioma]["tecla_right"],
    }

construir_nombres_teclas()
...
def actualizar_idioma():
    ...
    recorrer(ventana_acerca_de)
    crear_menu_icono()
    
    construir_nombres_teclas()
    clave = config['deshacer']['atajo_deshacer']
    if clave and clave != 'none':
        etiqueta_deshacer.config(text=nombres_teclas.get(clave, clave.upper()))
    crear_menu_icono()
    clave = config['atajo']['atajo']
    if clave and clave != 'none':
        partes = clave.split(" + ")
        etiqueta_atajo.config(text=" + ".join(nombres_teclas.get(p, p) for p in partes))
...
def guardar_atajo():                               # Guarda el atajo en config.ini y restaura la interfaz
    ...
    if teclas_atajo:
        ...
        for t in teclas_atajo:
            ...
            elif hasattr(t, 'char') and t.char:
                partes.append(t.char.lower())
            elif hasattr(t, 'vk') and t.vk in [226, 227]:
                partes.append("<" if t.vk == 226 else ">")
        config['atajo']['atajo'] = " + ".join(partes)
    else:
...
def limpiar_atajo():                               # Borra el atajo guardado
    teclas_atajo.clear()
    etiqueta_atajo.config(text=textos_idioma[idioma]["sin_atajo"])
    config['atajo']['atajo'] = 'none'
    ...
    for parte in partes:
        try:
            ...
            try:
                ...
                atajo = None
                return
    partes_bonitas = [nombres_teclas.get(p, p) for p in texto_atajo.split(" + ")]
    etiqueta_atajo.config(text=" + ".join(partes_bonitas))
...
def registrar_tecla_deshacer(tecla):               # Registra la tecla presionada durante la captura
    ...
    if hasattr(tecla, 'name') and tecla.name:
        etiqueta_deshacer.config(text=nombres_teclas.get(tecla.name, tecla.name))
...
def limpiar_deshacer_tecla():                      # Borra la tecla de deshacer asignada
    ...
    config['deshacer']['atajo_deshacer'] = 'none'
...
def guardar_tecla_deshacer():                      # Guarda la tecla en config.ini y restaura la interfaz
    ...
    print(teclas_atajo_deshacer)
    print(teclas_atajo_deshacer[0].name)
    if teclas_atajo_deshacer:
        config['deshacer']['atajo_deshacer'] = teclas_atajo_deshacer[0].name if hasattr(teclas_atajo_deshacer[0], 'name') and teclas_atajo_deshacer[0].name else teclas_atajo_deshacer[0].char
    else:
...
def iniciar_interfaz():
    ...
    tk.Label(frame_acerca_texto, text=NOMBRE, font=fuente_titulo, fg="#009000").pack(anchor="center")

    frame_version = tk.Frame(frame_acerca_texto)
    frame_version.pack(anchor="center")
    tk.Label(frame_version, text=textos_idioma[idioma]["version"]).grid(row=0, column=0)
    tk.Label(frame_version, text=VERSION).grid(row=0, column=1)

    frame_autor = tk.Frame(frame_acerca_texto)
    frame_autor.pack(anchor="center")
    tk.Label(frame_autor, text=textos_idioma[idioma]["autor"]).grid(row=0, column=0)
    tk.Label(frame_autor, text=AUTOR).grid(row=0, column=1)
    ...
    #Links
    frame_links = tk.Frame(ventana_acerca_de)
    frame_links.pack(pady=5)
    frame_links = tk.Frame(ventana_acerca_de)
    frame_links.pack(pady=(15,5))


    tk.Label(frame_links, text=textos_idioma[idioma]["pagina_web"]).grid(row=0, column=0, sticky="e")
    link_web = tk.Label(frame_links, text="https://github.com/Double-TK/Klavaranto", fg="blue", cursor="hand2")
    link_web.grid(row=0, column=1, sticky="w")
    link_web.bind("<Button-1>", lambda e: webbrowser.open("https://github.com/Double-TK/Klavaranto"))

    tk.Label(frame_links, text=textos_idioma[idioma]["reportar_error"]).grid(row=1, column=0, sticky="e")
    link_error = tk.Label(frame_links, text="https://github.com/Double-TK/Klavaranto/issues", fg="blue", cursor="hand2")
    link_error.grid(row=1, column=1, sticky="w")
    link_error.bind("<Button-1>", lambda e: webbrowser.open("https://github.com/Double-TK/Klavaranto/issues"))

    tk.Label(frame_links, text=textos_idioma[idioma]["sponsor"]).grid(row=2, column=0, sticky="e")
    link_sponsor = tk.Label(frame_links, text="https://ko-fi.com/doubletk", fg="blue", cursor="hand2")
    ...
    boton_acerca_izq = tk.Button(frame_acerca_botones, text=textos_idioma[idioma]["menu_opciones"], command=ventana_opciones.deiconify)
    boton_acerca_izq.pack(side="left")

    boton_acerca_der = tk.Button(frame_acerca_botones, text=textos_idioma[idioma]["cerrar"], command=ventana_acerca_de.withdraw)
